...
public class Hooks {
    ...
    public void beforeScenario(Scenario scenario) {
        ...
        ScenarioExecution scenarioExecution = ScenarioContext.getScenarioExecution();
        
        if (scenarioExecution != null && scenarioExecution.getExecution() != null) {
            Execution execution = scenarioExecution.getExecution();
            String browser = scenarioExecution.getBrowser();
            ...
            try {
                if (gridUrl != null && !gridUrl.isEmpty()) {
                    ...
                    scenarioExecution.setSessionId(((org.openqa.selenium.remote.RemoteWebDriver) driver).getSessionId().toString());
                    ...
                    com.bdd.portal.repository.ScenarioExecutionRepository repo = SpringContext.getBean(com.bdd.portal.repository.ScenarioExecutionRepository.class);
                    repo.save(scenarioExecution);
            ...
            log.info("Scenario Context not found. Initializing local DEV ChromeDriver.");
            org.openqa.selenium.chrome.ChromeOptions options = new org.openqa.selenium.chrome.ChromeOptions();
            options.addArguments("--remote-allow-origins=*");
            options.addArguments("disable-notifications");
            options.addArguments("start-maximized");
    ...
    public void afterScenario(Scenario scenario) {
        ...
        if (scenario.isFailed()) {
            StepLogger.log("[FAILED] Scenario execution failed.");
            try {
                ...
                java.lang.reflect.Field errorField = delegate.getClass().getDeclaredField("error");
                errorField.setAccessible(true);
                Throwable error = (Throwable) errorField.get(delegate);
                
                if (error != null) {
                    StepLogger.log("[ERROR REASON] " + error.getMessage());
                }
            } catch (Exception e) {
                StepLogger.log("[WARN] Could not extract failure reason from Scenario object (internal Cucumber structure may have changed).");
            }
        }
        ...
        try {
            ...
            log.error("Failed to take screenshot during teardown", e);
        } finally {
